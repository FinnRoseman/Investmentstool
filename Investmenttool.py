...
def generate_full_report():
    ...
    doc.add_paragraph("Zusammensetzung der Positionen:")
    pos_table = doc.add_table(rows=1, cols=2)
    ...
    for label, wert in werte_data:
        ...
        row[1].text = wert
    doc.add_page_break()
    doc.add_heading('2. Performance- und Risiko-Kennzahlen', level=1)
    kpi_table = doc.add_table(rows=1, cols=2)
    kpi_table.style = 'Table Grid'
    kpis = [
        ("Erwartete Rendite (CAPM)", f"{capm_erwartung_pa:.2%}"),
        ("Rendite p.a. (CAGR)", f"{cagr:.2%}"),
        ("Alpha (gegen Benchmark)", f"{alpha:.4f}"),
        ("Beta (Marktempfindlichkeit)", f"{beta:.2f}"),
        ("Maximum Drawdown", f"{max_drawdown:.2%}"),
        ("Volatilität p.a.", f"{vola:.2%}"),
        ("Tracking Error", f"{tracking_error:.2%}"),
        ("Dividendenrendite (Portfolio)", f"{port_current_yield:.2%}"),
        ("Yield on Cost", f"{port_yoc:.2%}"),
        ("Sharpe Ratio", f"{sharpe_ratio:.2f}"),
        ("Sortino Ratio", f"{sortino_ratio:.2f}"),
        ("Information Ratio", f"{information_ratio:.2f}"),
        ("Treynor Ratio", f"{treynor_ratio:.2f}"),
        ("Upside Capture Ratio", f"{upside_ratio:.2f}%"),
        ("Downside Capture Ratio", f"{downside_ratio:.2f}%")
    ]
    for k_label, k_val in kpis:
        row = kpi_table.add_row().cells
        row[0].text = k_label
        row[1].text = k_val
    doc.add_paragraph("\n*Hinweis: Alle Kennzahlen basieren auf dem gewählten Betrachtungszeitraum.")
    doc.add_page_break()
    doc.add_heading('3. Historische Performance-Entwicklung', level=1)
    doc.add_paragraph("Der folgende Chart zeigt die kumulierte Wertentwicklung deines Portfolios im Vergleich zur gewählten Benchmark über den gesamten Zeitraum.")
    add_plot_to_word(doc, fig_perf)
    doc.add_page_break()
    doc.add_heading('4. Geografische Allokation', level=1)
    doc.add_paragraph("Die regionale Verteilung basierend auf deinen Eingaben:")
    if 'fig_donut' in locals() or 'fig_donut' in globals():
        add_plot_to_word(doc, fig_donut, width=4.5)
    else:
        doc.add_paragraph("Keine regionalen Daten verfügbar.")
    doc.add_paragraph("")
    doc.add_heading('5. Dividenden-Kalender & Historie', level=1)
    if not div_df.empty:
        doc.add_paragraph("Übersicht der letzten bzw. anstehenden Dividendenzahlungen der Portfolio-Positionen:")
        div_table = doc.add_table(rows=1, cols=3)
        div_table.style = 'Table Grid'
        hdr = div_table.rows[0].cells
        hdr[0].text = 'Ticker'
        hdr[1].text = 'Datum'
        hdr[2].text = 'Betrag'
        for _, row in div_df.head(15).iterrows():
            c = div_table.add_row().cells
            c[0].text = str(row['Ticker'])
            c[1].text = str(row['Date'])
            c[2].text = f"{row['Dividends']:.2f}"
    else:
        doc.add_paragraph("Für diesen Zeitraum wurden keine Dividendendaten gefunden.")
    doc.add_page_break()
    doc.add_heading('6. Fama-French Faktorenanalyse', level=1)
    doc.add_paragraph("Diese Analyse zeigt, welche Risikofaktoren (Market, Size, Value, Profitability, Investment) die Rendite deines Portfolios maßgeblich beeinflussen.")
    if 'fig_fac' in locals() or 'fig_fac' in globals():
        add_plot_to_word(doc, fig_fac)
    else:
        doc.add_paragraph("Die Faktorenanalyse konnte für diesen Zeitraum nicht berechnet werden.")
    doc.add_page_break()
    doc.add_heading('7. Detaillierte Renditebetrachtung', level=1)
    doc.add_paragraph("Vergleich der Wertentwicklung auf kummulierter Basis (Gesamtrendite) und der durchschnittlichen jährlichen Wachstumsrate (CAGR).")
    if 'fig_yearly' in locals() or 'fig_yearly' in globals():
        add_plot_to_word(doc, fig_yearly)
    ret_table = doc.add_table(rows=1, cols=2)
    ret_table.style = 'Light List Accent 1'
    ret_data = [
        ("Kumulierte Rendite Portfolio", f"{rel_gewinn:.2%}"),
        ("Annualisierte Rendite (CAGR)", f"{cagr:.2%}"),
        ("Kumulierte Rendite Benchmark", f"{(bench_final/bench_start - 1):.2%}" if 'bench_final' in locals() else "N/A")
    ]
    for label, val in ret_data:
        row = ret_table.add_row().cells
        row[0].text, row[1].text = label, val 
    doc.add_page_break()
    doc.add_heading('8. Rolling Returns (12-Monats-Fenster)', level=1)
    doc.add_paragraph("Die Rolling Returns zeigen die annualisierte Rendite über gleitende 12-Monats-Zeiträume. Dies hilft dabei, die Beständigkeit der Performance zu bewerten und Phasen von Underperformance zu identifizieren.")
    if 'fig_rolling' in locals() or 'fig_rolling' in globals():
        add_plot_to_word(doc, fig_rolling)
    else:
        doc.add_paragraph("Rolling Returns konnten nicht berechnet werden (Zeitraum evtl. zu kurz).")
    doc.add_paragraph("")
    doc.add_heading('9. Renditeverteilung (Histogramm)', level=1)
    doc.add_paragraph("Das Histogramm zeigt die Häufigkeitsverteilung der täglichen Renditen. Eine Glockenkurve (Normalverteilung) ist das theoretische Modell, während Abweichungen (Fat Tails) auf erhöhte Risiken hinweisen.")
    if 'fig_dist' in locals() or 'fig_dist' in globals():
        add_plot_to_word(doc, fig_dist)
    doc.add_page_break()
    doc.add_heading('10. Korrelationsmatrix', level=1)
    doc.add_paragraph("Die Korrelationsmatrix zeigt den linearen Zusammenhang zwischen den einzelnen Positionen. Werte nahe +1 bedeuten einen starken Gleichlauf, Werte nahe 0 eine gute Diversifikation.")
    if 'fig_corr' in locals() or 'fig_corr' in globals():
        add_plot_to_word(doc, fig_corr)
    else:
        doc.add_paragraph("Korrelationsmatrix nicht verfügbar.")
    doc.add_page_break()
    doc.add_heading('11. Risikoverteilung (Risk Contribution)', level=1)
    doc.add_paragraph("Diese Analyse zeigt, wie viel Prozent der Gesamtvolatilität des Portfolios auf die jeweilige Einzelposition zurückzuführen ist.")
    if 'fig_bar' in locals() or 'fig_bar' in globals():
        add_plot_to_word(doc, fig_bar)
    else:
        doc.add_paragraph("Risikoverteilungs-Chart nicht verfügbar.") 
    doc.add_paragraph("")
    doc.add_heading('12. Value at Risk (VaR) & Expected Shortfall (ES)', level=1)
    doc.add_paragraph("Der Value at Risk (95%) gibt an, welcher Verlust innerhalb eines Jahres mit einer Wahrscheinlichkeit von 95% nicht überschritten wird. Der Expected Shortfall beschreibt den durchschnittlichen Verlust, falls das VaR-Ereignis eintritt.")
    risk_table = doc.add_table(rows=1, cols=3)
    risk_table.style = 'Medium Shading 1 Accent 1'
    hdr_cells = risk_table.rows[0].cells
    hdr_cells[0].text = 'Methode'
    hdr_cells[1].text = 'VaR 95% (p.a.)'
    hdr_cells[2].text = 'Exp. Shortfall (p.a.)'
    risk_data = [
        ("Parametrisch", f"{var_95_para:.2%}", f"{es_95_para:.2%}"),
        ("Historisch", f"{var_95_hist:.2%}", f"{es_95_hist:.2%}"),
        ("Monte Carlo", f"{mc_var_95_jahr:.2%}", f"{mc_es_95_jahr:.2%}")
    ]
    for methode, var_val, es_val in risk_data:
        row_cells = risk_table.add_row().cells
        row_cells[0].text = methode
        row_cells[1].text = var_val
        row_cells[2].text = es_val
    doc.add_paragraph("\n*NTM (Next Twelve Months): Die Werte sind auf ein Jahr hochgerechnet.")
    doc.add_page_break()
    doc.add_heading('13. Mean-Variance Optimization (Efficient Frontier)', level=1)
    doc.add_paragraph("Die Simulation zeigt 5.000 theoretisch mögliche Portfolio-Kombinationen. Der rote Stern markiert das Portfolio mit der höchsten Sharpe-Ratio (Maximum Sharpe Portfolio), der gelbe Stern das Portfolio mit dem geringsten Risiko (Minimum Variance Portfolio).")
    if 'fig_ef' in locals() or 'fig_ef' in globals():
        add_plot_to_word(doc, fig_ef)
        doc.add_paragraph(f"Optimierungsergebnis: Das Portfolio mit der maximalen Sharpe-Ratio erreicht theoretisch eine Rendite von {max_sharpe_return:.2%}, während das Minimum-Variance-Portfolio das Risiko auf {min_vol_volatility:.2%} senkt.")
    else:
        doc.add_paragraph("Optimierungs-Simulation wurde nicht durchgeführt.")
    doc.add_page_break()
    doc.add_heading('14. Monte Carlo Pfadsimulation (10 Jahre)', level=1)
    doc.add_paragraph("Basierend auf der historischen Volatilität und Rendite wurden 1.000 mögliche Zukunftsverläufe für die nächsten 10 Jahre simuliert. Dies visualisiert die Bandbreite möglicher Endvermögenswerte.")
    if 'fig_mc_path' in locals() or 'fig_mc_path' in globals():
        add_plot_to_word(doc, fig_mc_path)
        doc.add_heading('Statistische Auswertung der Simulation (Endwerte):', level=2)
        mc_table = doc.add_table(rows=1, cols=2)
        mc_table.style = 'Light List Accent 1'
        mc_data = [
            ("Optimistisches Szenario (95% Perzentil)", f"{np.percentile(ergebnisse_mc, 95):,.2f} €"),
            ("Erwarteter Wert (Median)", f"{np.percentile(ergebnisse_mc, 50):,.2f} €"),
            ("Pessimistisches Szenario (5% Perzentil)", f"{np.percentile(ergebnisse_mc, 5):,.2f} €")
        ]
        for label, wert in mc_data:
            row = mc_table.add_row().cells
            row[0].text = label
            row[1].text = wert
    else:
        doc.add_paragraph("Pfadsimulation nicht verfügbar.")
    doc.add_page_break()
